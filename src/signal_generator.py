...
def generate_signals(df_primary, df_higher, symbol):
    ...
    if (prev_row['ema_short'] <= prev_row['ema_medium'] and 
        latest_row['ema_short'] > latest_row['ema_medium'] and 
        abs(latest_row['ema_short'] - latest_row['ema_medium']) / current_price > 0.002):
        buy_factors.add('ema')
    ...
    if latest_row['price_change'] > 0.005 and latest_row['close'] > latest_row['open']:
        buy_factors.add('price_action')
        buy_reasons.append(f"کندل صعودی قوی (+{latest_row['price_change']*100:.2f}%)")

    if higher_tf_trend == 'up' or trend == 'neutral':
    ...
    if higher_tf_trend == 'down' or trend == 'neutral':
